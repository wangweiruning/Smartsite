...
import {Text,View,Image,RefreshControl,StyleSheet,ScrollView,ToastAndroid} from 'react-native';
...
export default class Personnel extends React.Component {
  ...
  constructor(props){
    ...
    getMonthData= async()=>{
        let datas = await data_month(this.state.http);
        let daydatas = await data_day(this.state.http);
console.log(datas,daydatas)
  ...
  render() {
    ...
    const tabs = [
      ...
      return (<View style={{flex:1}}>
               <HeaderBar gets={this.goflash.bind(this)} flashing={true} parent={this} name="人员统计" rightIcon={'sync'}/>
               ...
               <View style={styles.content}>
                    <View style={styles.wuzi}>
                        ...
                        <Text>人，工程总人数</Text>
                        <Text style={{fontSize:20,color:'#4dd0e1'}}>{this.state.AllPersonNum}</Text>
                        <Text>人</Text>
                    </View>
                    <View style={styles.tongji}>
                        <View style={styles.Wireframe}>
                            ...
                            <View style={styles.showPerson}>
                                <Text style={{fontSize:18}}>进站</Text>
