...
import {createTicket,uploadEleImage} from '../../api/api'
...
export default class CreateEticket extends React.Component {
  ...
  Mygets(v,keys){
      console.log(v,"vvvvvvvvvvvvvvvvv")
        this.handleInput('responsePerson',v.userid)
}
    submit = async() => {
        let {submitData,avatarSource} = this.state;
        ...
        console.log(submitData,"bbbbbbbbbbbbbb")
        if (arr.indexOf('') != -1 || arr.indexOf(null) != -1 ){
            ToastAndroid.show('请将数据填写完整',ToastAndroid.SHORT)
         }else{
            let res = await createTicket(this.state.http,{...submitData,attachId:picids.join(',')});
            console.log('创建电子作业票===========',res)
            if(res.code=='S10000'){
                ToastAndroid.show('电子作业票创建成功',ToastAndroid.SHORT)
...
opens(v){//接收子组件传递过来的数据
    ...
    this.handleInput('participant',tt.join(','));
}
...
choosePic(mediaType,pp,tt) {
    ...
    ImagePicker2.showImagePicker({
      ...
      if (response.didCancel) {
        ...
        if(mediaType=='photo'){
          ...
          //获取图片的id
          let unload  = await uploadEleImage(fromdate);
  ...
  render() {
      return (<View style={{flex:1,backgroundColor:"#FFF"}}>
               <HeaderBar   parent={this} name="电子作业票创建"/>
               ...
               <ScrollView>
                    {!this.state.loading&&<View style={{justifyContent:'center',alignContent:'center',flexDirection:'column',alignItems:'center',width:'100%'}}>
                        ...
                        {/* <View style={styles.modelsContent}>
                                <Text style={{width:80,color:"#000",fontSize:13}}>关联工程*</Text>
                            ...
                            <View style={styles.modelsContent}>
                                <Text style={{width:80,color:"#000",fontSize:13}}>电子票编号*</Text>
                                <View style={styles.inputValves}>
                                <TextInput placeholder='请输入电子票编号'
                                                underlineColorAndroid={'transparent'}
                                                style={[styles.inputitem]}
                                                placeholderTextColor={"#4e4e4e"}
                                                onChangeText={(e)=>this.handleInput('ticketNumber',e)}/>
                                </View>
                            </View>
                            <View style={styles.modelsContent}>
                                <Text style={{width:80,color:"#000",fontSize:13}}>作业部位*</Text>
                                <View style={styles.inputValves}>
                                <TextInput placeholder='请填写作业部位'
                                                underlineColorAndroid={'transparent'}
                                                style={[styles.inputitem]}
                                                placeholderTextColor={"#4e4e4e"}
                                                onChangeText={(e)=>this.handleInput('position',e)}/>
                                </View>
                            </View>
                            <View style={styles.modelsContent}>
                                <Text style={{width:80,color:"#000",fontSize:13}}>作业内容*</Text>
                                <View style={styles.inputValves}>
                                    <TextareaItem rows={6} 
                                         ...
                                         minWidth:'75%',maxWidth:'75%',borderRadius:5}} placeholder="请填写工序及作业内容"/>
                                </View>
                            </View>
                            <View style={styles.modelsContent}>
