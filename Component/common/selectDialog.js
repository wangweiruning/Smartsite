...
export default class SelectDialog extends Component{
			...
			return(
				...
				<Modal
					...
					<View style={[styles.container,styles.flexVer,styles.flex1]}>
					  <TouchableOpacity onPress={this.hide.bind(this)} style={[styles.mask]} underlayColor='transparent'>
						<Text></Text>
					  </TouchableOpacity>
					  ...
					  {width:this.innersWidth,height:this.innersHeight,}]}>
						   ...
						   <View style={[styles.flexRow,styles.flex1]}>
							 <ScrollView style={[styles.flex1]}>
							  {datas.map((item,i)=>this.createList(item,i,this.props.keys)) }
							 </ScrollView>
						   </View>
					  </View>
					</View>
	...
	setNum(item,index,key){
		this.setState({selectNum:index})
		this.props.valueChange(item,index,key)
		this.hide()
	}
	createList(item,i,keys){
		return(
		  <CreatList index={i} innerW={this.innersWidth} sel={this.state.selectNum===i} renderRow={this.props.renderRow} key={i} keys={this.props.keys}  SetNum={this.setNum.bind(this)} items={item} />
		)
	}
	hide(){
...
class CreatList extends Component{
	...
	setNum(item,index,keys){
		this.props.SetNum(item,index,keys)
	}
	render(){
		return(
		  <TouchableOpacity  
			 ...
			 onPress={()=>this.setNum(this.props.items,this.props.index,this.props.keys)}>
		   {!this.props.renderRow ? (<Text style={[
			  ...
			  styles.h28,
			   ...
			   {this.props.items.name||this.props.items.type}
			 </Text> ):
